...
$(document).ready(function() {
    ...
    function resetUI() {
        ...
        $('#invoice-batch-list, #quotation-details, #quotation-items-table').html('');
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                data.order_items.forEach(item => {
                    ...
                    itemsHtml += `<tr><td>${item.inventory_name} ${lokasi}</td><td>${item.mkt_quotation_order_item_qty}</td><td>${formatter.format(item.mkt_quotation_order_item_price)}</td><td>${formatter.format(item.mkt_quotation_order_item_qty * item.mkt_quotation_order_item_price)}</td></tr>`;
                });
                $('#quotation-items-table').html(itemsHtml);
    ...
    // Tambahkan event handler ini di dalam $(document).ready()
        ...
        $.ajax({
            ...
            success: function(response) {
                if(response.status === 'success') {
                    ...
                    window.location.reload(); // Atau redirect ke halaman daftar invoice
                } else {
